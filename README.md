## Hi there 👋 I'm Drew Grauch

### **Industrial Engineering @ Georgia Tech**

### **Highlights**
* **Incoming Area Manager Intern @ Amazon (Summer 2026):** Focusing on high-volume logistics and warehouse operations strategy
* **Global Immersion:** Completed a technical exchange at **Yonsei University** in Seoul and I'm part of GA Tech's International Plan
* **Technical Focus:** Relational database architecture, Multi-objective Integer Programming, and Data Analytics

### **Tools**
* **Languages:** Python (Pandas, NumPy), SQL (MySQL, Stored Procedures), R
* **Optimization:** Gurobi Optimizer, Linear/Integer Programming, Stochastic Systems
* **Tools:** Git/GitHub, Jupyter

### **📈 Featured Projects**
* **[Bus Fleet Optimization](https://github.com/jgrauch3/bus-fleet-route-optimization):** Multi-objective Gurobi model balancing transit efficiency with system fairness
* **[Media Streaming Backend](https://github.com/jgrauch3/media-streaming-database):** 20+ table RDBMS architecture with automated SQL stored procedures
* **[GA School Statistics Analytics](https://github.com/jgrauch3/public-school-statistics-analytics):** End-to-end data study comparing regional performance to national benchmarks

---

[LinkedIn](https://www.linkedin.com/in/drew-grauch-2b32a0292/) | [Email](mailto:Josie.Drew33@gmail.com)

<!--
**jgrauch3/jgrauch3** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
